...
set(_exported_dependencies "rclcpp;rclcpp_action;rclcpp_lifecycle;std_msgs;visualization_msgs;nav2_util;nav2_msgs;nav_msgs;geometry_msgs;builtin_interfaces;tf2_ros;nav2_costmap_2d;nav2_core;pluginlib;dynamicvoronoi")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to nav2_voronoi_planner_DEFINITIONS, nav2_voronoi_planner_INCLUDE_DIRS,
# nav2_voronoi_planner_LIBRARIES, and nav2_voronoi_planner_LINK_FLAGS.
# Additionally collect the direct dependency names in
# nav2_voronoi_planner_DEPENDENCIES as well as the recursive dependency names
# in nav2_voronoi_planner_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(nav2_voronoi_planner_DEPENDENCIES ${_exported_dependencies})
  set(nav2_voronoi_planner_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(nav2_voronoi_planner_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(nav2_voronoi_planner_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(nav2_voronoi_planner_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(nav2_voronoi_planner_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(nav2_voronoi_planner_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND nav2_voronoi_planner_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
