...
If you just want to open the webcam and directly inspect XFeat keypoint localization without setting a reference frame, use:
```bash
python3 webcam_xfeat_points.py
```

You can also tune the number of points and filtering threshold, for example:
```bash
python3 webcam_xfeat_points.py --max_kpts 1000 --detection_threshold 0.03 --min_score 0.1
```

If you want an Intel RealSense RGB-D demo that uses the color stream for XFeat and shows aligned depth at the detected keypoints, use:
```bash
python3 realsense_xfeat_rgbd.py
```

This RealSense demo also includes a basic RGB-D visual odometry mode: it matches XFeat features between consecutive frames, uses depth to back-project 3D points, estimates camera pose with PnP-RANSAC, and shows a top-down trajectory panel in real time.
