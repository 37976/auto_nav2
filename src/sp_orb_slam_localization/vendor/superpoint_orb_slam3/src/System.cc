    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    {
        cout << "Minimal mode disables atlas serialization; ignoring load/save atlas settings." << endl;
        mStrLoadAtlasFromFile.clear();
        mStrSaveAtlasToFile.clear();
    }

    mpVocabulary = nullptr;
    mpKeyFrameDatabase = nullptr;
    cout << "Initialization of Atlas from scratch (minimal mode)" << endl;
    mpAtlas = new Atlas(0);
#else
...
#else
        cout << "Minimal mode disables the Pangolin viewer." << endl;
#endif
    }
...
{
    std::cout << "SaveTrajectoryTUM begin" << std::endl;
    std::cout << "mlRelativeFramePoses size = " << mpTracker->mlRelativeFramePoses.size() << std::endl;
    std::cout << "mlpReferences size       = " << mpTracker->mlpReferences.size() << std::endl;
    std::cout << "mlFrameTimes size        = " << mpTracker->mlFrameTimes.size() << std::endl;
    std::cout << "mlbLost size             = " << mpTracker->mlbLost.size() << std::endl;
    
    size_t n1 = mpTracker->mlRelativeFramePoses.size();
    size_t n2 = mpTracker->mlpReferences.size();
    size_t n3 = mpTracker->mlFrameTimes.size();
    size_t n4 = mpTracker->mlbLost.size();

    if(!(n1 == n2 && n2 == n3 && n3 == n4))
    {
        std::cout << "WARNING: tracker lists have different sizes!" << std::endl;
    }
    ...
    cout << "SaveTrajectoryTUM begin" << endl;
    cout << "mlRelativeFramePoses size = " << mpTracker->mlRelativeFramePoses.size() << endl;
    cout << "mlpReferences size       = " << mpTracker->mlpReferences.size() << endl;
    cout << "mlFrameTimes size        = " << mpTracker->mlFrameTimes.size() << endl;
    cout << "mlbLost size             = " << mpTracker->mlbLost.size() << endl;
    cout << "vpKFs size               = " << vpKFs.size() << endl;

    if(vpKFs.empty())
    {
        cerr << "SaveTrajectoryTUM skipped: vpKFs is empty." << endl;
        return;
    }

    if(mpTracker->mlRelativeFramePoses.empty() ||
    mpTracker->mlpReferences.empty() ||
    mpTracker->mlFrameTimes.empty() ||
    mpTracker->mlbLost.empty())
    {
        cerr << "SaveTrajectoryTUM skipped: tracking history is empty." << endl;
        return;
    }
    ...
    for( ; lit != mpTracker->mlRelativeFramePoses.end()
...
#ifdef ORB_SLAM3_MINIMAL
    (void)type;
    cout << "Atlas load is disabled in minimal mode." << endl;
    return false;
#else
