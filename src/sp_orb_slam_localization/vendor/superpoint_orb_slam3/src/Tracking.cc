...
#ifndef ORB_SLAM3_DEFAULT_SP_MODEL_PATH
#define ORB_SLAM3_DEFAULT_SP_MODEL_PATH "weights/superpoint.ts"
#endif

namespace {

std::string ResolveSuperPointModelPath()
{
    const char* env_path = std::getenv("ORB_SLAM3_SP_MODEL_PATH");
    if(env_path && env_path[0] != '\0')
        return std::string(env_path);

    return ORB_SLAM3_DEFAULT_SP_MODEL_PATH;
}

constexpr int kEarlyRgbdStabilizationFrames = 20;
...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    int spLevels = 3;   // SuperPoint 固定单层
    int fIniThFAST = settings->initThFAST();
    int fMinThFAST = settings->minThFAST();
    float fScaleFactor = settings->scaleFactor();

    mbUseSuperPoint = true;
    mSPModelPath = ResolveSuperPointModelPath();
    mSPConfTh = 0.003f;

    if(mbUseSuperPoint)
    {
        mpFeatureExtractorLeft =
            new SuperPointExtractor(mSPModelPath, nFeatures, fScaleFactor, spLevels, mSPConfTh);
        ...
        if(mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR)
            mpIniFeatureExtractor =
                new SuperPointExtractor(mSPModelPath, 5*nFeatures, fScaleFactor, spLevels, mSPConfTh);
    }
    else
    {
        mpFeatureExtractorLeft =
            new ORBextractor(nFeatures,fScaleFactor,spLevels,fIniThFAST,fMinThFAST);
        ...
        if(mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR)
            mpIniFeatureExtractor =
                new ORBextractor(5*nFeatures,fScaleFactor,spLevels,fIniThFAST,fMinThFAST);
    }
...
bool Tracking::ParseORBParamFile(cv::FileStorage &fSettings)
{
    ...
    {
        mpFeatureExtractorLeft =
            new SuperPointExtractor(mSPModelPath, nFeatures, fScaleFactor, nLevels, mSPConfTh);
        ...
        if(mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR)
            mpIniFeatureExtractor =
                new SuperPointExtractor(mSPModelPath, 5*nFeatures, fScaleFactor, nLevels, mSPConfTh);
    }
    else
    {
        mpFeatureExtractorLeft =
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    if (mSensor == System::STEREO && !mpCamera2)
        mCurrentFrame = Frame(mImGray,imGrayRight,timestamp,
                            mpFeatureExtractorLeft,mpFeatureExtractorRight,
...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    if (mSensor == System::RGBD)
        mCurrentFrame = Frame(mImGray,imDepth,timestamp,mpFeatureExtractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
    else if(mSensor == System::IMU_RGBD)
        mCurrentFrame = Frame(mImGray,imDepth,timestamp,mpFeatureExtractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,&mLastFrame,*mpImuCalib);
    std::cout << "[GrabImageRGBD] N=" << mCurrentFrame.N
          << " mvKeys=" << mCurrentFrame.mvKeys.size()
          << " desc=" << mCurrentFrame.mDescriptors.rows << "x"
          << mCurrentFrame.mDescriptors.cols << std::endl;
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        std::cout << "[Track] state=" << mState
          << " mnId=" << mCurrentFrame.mnId
          << " N=" << mCurrentFrame.N
          << " refKF=" << (mCurrentFrame.mpReferenceKF ? 1 : 0)
          << " pose=" << (mCurrentFrame.isSet() ? 1 : 0)
          << std::endl;
        // Update drawer
        mpFrameDrawer->Update(this);
        ...
        if(mCurrentFrame.mpReferenceKF)
        {
            Sophus::SE3f Tcr = mCurrentFrame.GetPose() * mCurrentFrame.mpReferenceKF->GetPoseInverse();
...
{
    int minInitFeatures = mbUseSuperPoint ? 70 : 500;

    if(mCurrentFrame.N > minInitFeatures)
    {
        int nValidDepth = 0;
        for(int i = 0; i < mCurrentFrame.N; i++)
        {
        ...
        std::cout << "[StereoInitialization] N=" << mCurrentFrame.N
                << " validDepth=" << nValidDepth << std::endl;
...
if(mCurrentFrame.N>500)
    ...
    {
        std::cout << "[StereoInitialization] skip, N=" << mCurrentFrame.N
                  << " < minInitFeatures=" << minInitFeatures << std::endl;
    }
}
...
{
    constexpr int kMinInitFeatures = 80;
    constexpr int kMinInitMatches = 60;
    constexpr int kInitSearchWindow = 120;

    if(!mbReadyToInitializate)
    {
        // Set Reference Frame
        if(static_cast<int>(mCurrentFrame.mvKeys.size()) > kMinInitFeatures)
        {
    ...
    {
        if (((int)mCurrentFrame.mvKeys.size() <= kMinInitFeatures) ||
        ...
        SuperPointMatcher matcher(0.9f,false);
        int nmatches = matcher.SearchForInitialization(
            mInitialFrame, mCurrentFrame, mvbPrevMatched, mvIniMatches, kInitSearchWindow);

        std::cout << "[MonoInit] features0=" << mInitialFrame.mvKeys.size()
                  << " features1=" << mCurrentFrame.mvKeys.size()
                  << " matches=" << nmatches
                  << " min_matches=" << kMinInitMatches
                  << std::endl;
        ...
        {
            std::cout << "[MonoInit] ReconstructWithTwoViews failed" << std::endl;
            mbReadyToInitializate = false;
        }
    }
}
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    if(mInitialFrame.mDescriptors.type() == CV_8U &&
    mCurrentFrame.mDescriptors.type() == CV_8U)
    {
        if(mInitialFrame.mDescriptors.type() == CV_8U &&
        mCurrentFrame.mDescriptors.type() == CV_8U)
        {
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    SuperPointMatcher matcher(0.8f, false);

    int nmatches = matcher.SearchByProjection(
        mCurrentFrame, mpReferenceKF, sAlreadyFound, 10.0f, 1.0f);
    ...
    if((mSensor == System::RGBD || mSensor == System::IMU_RGBD) &&
       mCurrentFrame.mnId <= mnLastKeyFrameId + kEarlyRgbdStabilizationFrames)
    {
        std::cout << "[RGBD] Skip PoseOptimization in TrackReferenceKeyFrame during early stabilization window"
                  << " frameId=" << mCurrentFrame.mnId
                  << " lastKF=" << mnLastKeyFrameId
...
void Tracking::UpdateLastFrame()
{
    ...
    if(mSensor == System::RGBD || mSensor == System::IMU_RGBD)
    {
        std::cout << "[RGBD] Skip temporal VO MapPoint creation in UpdateLastFrame while only-tracking is active"
                  << " frameId=" << mLastFrame.mnId << std::endl;
        return;
    }
...
bool Tracking::TrackWithMotionModel()
{
    ...
    if((mSensor == System::RGBD || mSensor == System::IMU_RGBD) &&
       mCurrentFrame.mnId <= mnLastKeyFrameId + kEarlyRgbdStabilizationFrames)
    {
        std::cout << "[RGBD] Skip PoseOptimization in TrackWithMotionModel during early stabilization window"
                  << " frameId=" << mCurrentFrame.mnId
                  << " lastKF=" << mnLastKeyFrameId
                  << " matches=" << nmatches << std::endl;
        return true;
...
bool Tracking::TrackLocalMap()
{
    ...
    if ((mSensor == System::RGBD || mSensor == System::IMU_RGBD) &&
        mCurrentFrame.mnId <= mnLastKeyFrameId + kEarlyRgbdStabilizationFrames)
    {
        std::cout << "[RGBD] Skip PoseOptimization in TrackLocalMap during early stabilization window"
                  << " frameId=" << mCurrentFrame.mnId
                  << " lastKF=" << mnLastKeyFrameId << std::endl;
    }
    ...
    {
        int minLocalMapInliers = 30;

        if(mbUseSuperPoint)
            minLocalMapInliers = 15;   // 先从 15 开始试

        if(mnMatchesInliers < minLocalMapInliers)
            return false;
        else
            return true;
    }
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    if(mbUseSuperPoint)
        bNeedToInsertClose = (nTrackedClose < 40) && (nNonTrackedClose > 20);
    else
...
{   
#ifdef ORB_SLAM3_MINIMAL
    return false;
#endif
    if(!mpKeyFrameDB)
        return false;
    if(mCurrentFrame.mDescriptors.type() != CV_8U)
    return false;
