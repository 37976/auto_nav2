...
namespace
{
bool IsFinitePoint2(const cv::Point2f& pt)
{
    return std::isfinite(pt.x) && std::isfinite(pt.y);
}

bool IsFiniteVec3(const Eigen::Vector3f& v)
{
    return v.allFinite() && v.norm() < 1e4f;
}
}
...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    if(!Tcw.translation().allFinite())
    {
        std::cout << "[RGBD] PoseOptimization skipped: non-finite initial pose translation" << std::endl;
        return 0;
    }
    vSE3->setEstimate(g2o::SE3Quat(Tcw.unit_quaternion().cast<double>(),Tcw.translation().cast<double>()));
    vSE3->setId(0);
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP)
        {
            if(pMP->isBad())
            {
                pFrame->mvpMapPoints[i] = static_cast<MapPoint*>(nullptr);
                continue;
            }

            const Eigen::Vector3f Xwf = pMP->GetWorldPos();
            if(!IsFiniteVec3(Xwf))
            {
                pFrame->mvpMapPoints[i] = static_cast<MapPoint*>(nullptr);
                continue;
            ...
            if(!pFrame->mpCamera2){
                ...
                if(pFrame->mvuRight[i]<0)
                {
                    ...
                    if(kpUn.octave < 0 || static_cast<size_t>(kpUn.octave) >= pFrame->mvInvLevelSigma2.size() || !IsFinitePoint2(kpUn.pt))
                    {
                        nInitialCorrespondences--;
                ...
                else  // Stereo observation
                {
                    ...
                    if(kpUn.octave < 0 || static_cast<size_t>(kpUn.octave) >= pFrame->mvInvLevelSigma2.size() ||
                       !IsFinitePoint2(kpUn.pt) || !std::isfinite(kp_ur))
                    {
                        nInitialCorrespondences--;
                        pFrame->mvbOutlier[i] = true;
                        continue;
    ...
    const size_t nPoseEdges = vpEdgesMono.size() + vpEdgesMono_FHR.size() + vpEdgesStereo.size();
    if(nInitialCorrespondences<3 || nPoseEdges < 10)
    {
        if(nPoseEdges < 10)
        {
            std::cout << "[RGBD] PoseOptimization skipped: too few valid edges=" << nPoseEdges
                      << " correspondences=" << nInitialCorrespondences << std::endl;
        }
        return 0;
    }
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        if(!Tcw.translation().allFinite())
        {
            std::cout << "[RGBD] PoseOptimization aborted: pose became non-finite before iteration " << it << std::endl;
            return 0;
        }
        vSE3->setEstimate(g2o::SE3Quat(Tcw.unit_quaternion().cast<double>(),Tcw.translation().cast<double>()));
    ...
    {
        std::cout << "[RGBD] PoseOptimization produced non-finite pose, keeping previous estimate" << std::endl;
        return 0;
    }
    pFrame->SetPose(pose);
