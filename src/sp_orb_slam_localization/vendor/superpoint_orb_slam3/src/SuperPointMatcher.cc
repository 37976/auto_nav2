...
static const float TH_SP_HIGH = 0.80f;
static const float TH_SP_LOW  = 0.70f;
using namespace std;
    ...
    for (int i = 0; i < LastFrame.N; i++)
    {
        ...
        if (pMP)
        {
            if (!LastFrame.mvbOutlier[i])
            {
                ...
                if (bestIdx2 >= 0 && bestDist < TH_SP_HIGH)
                {
...
int SuperPointMatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
{
    ...
    for(size_t i1 = 0, iend1 = F1.mvKeysUn.size(); i1 < iend1; i1++)
    {
        ...
        const vector<size_t> vIndices2 = F2.GetFeaturesInArea(
            vbPrevMatched[i1].x, vbPrevMatched[i1].y, windowSize, level1, level1);

        if(vIndices2.empty())
            continue;

        const cv::Mat d1 = F1.mDescriptors.row(static_cast<int>(i1));
        ...
        {
            const size_t i2 = *vit;
            const cv::Mat d2 = F2.mDescriptors.row(static_cast<int>(i2));
        ...
        if(bestIdx2 >= 0 && bestDist < TH_SP_HIGH && bestDist < bestDist2 * mfNNratio)
        {
...
int SuperPointMatcher::SearchBySim3(KeyFrame* pKF1, KeyFrame* pKF2, vector<MapPoint *> &vpMatches12, const Sophus::Sim3f &S12, const float th)
{
    ...
    for (int i1 = 0; i1 < N1; i1++)
    {
        ...
        if (bestIdx >= 0 && bestDist < TH_SP_HIGH)
        {
            vnMatch1[i1] = bestIdx;
        }
    }
