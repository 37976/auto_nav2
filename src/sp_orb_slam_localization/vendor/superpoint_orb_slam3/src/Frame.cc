      ...
      mbHasPose(false), mbHasVelocity(false)
{
    ...
    void Frame::ExtractFeatures(int flag, const cv::Mat &im, const int x0, const int x1)
    {
        ...
        if(flag==0)
        {
            std::cout << "[ExtractFeatures] LEFT: mvKeys=" << mvKeys.size()
                    << " desc=" << mDescriptors.rows << "x" << mDescriptors.cols
                    << " type=" << mDescriptors.type() << std::endl;
        }
        else
        {
            std::cout << "[ExtractFeatures] RIGHT/DEPTH side: mvKeysRight=" << mvKeysRight.size()
                    << " descR=" << mDescriptorsRight.rows << "x" << mDescriptorsRight.cols
                    << " type=" << mDescriptorsRight.type() << std::endl;
        }
    }
...
void Frame::ComputeBoW()
{
    if(mBowVec.empty())
    {
        ...
        // SuperPoint 是 CV_32F，先不要送进 ORB vocabulary
        if(mDescriptors.type() != CV_8U)
            return;
...
void Frame::ComputeStereoMatches()
{
    ...
    const bool useSP =
        (!mDescriptors.empty() && !mDescriptorsRight.empty() &&
        mDescriptors.type() == CV_32F && mDescriptorsRight.type() == CV_32F);

    const int thOrbDist = (ORBmatcher::TH_HIGH + ORBmatcher::TH_LOW) / 2;
    const float thSPDist = 0.70f;
    ...
    for(int iL=0; iL<N; iL++)
    {
        ...
        float bestDist = useSP ? std::numeric_limits<float>::max()
                       : static_cast<float>(ORBmatcher::TH_HIGH);
        size_t bestIdxR = 0;
        ...
        for(size_t iC=0; iC<vCandidates.size(); iC++)
        {
            ...
            if(uR>=minU && uR<=maxU)
            {
                ...
                float dist = 0.0f;
                if(useSP)
                    dist = SuperPointMatcher::DescriptorDistance(dL, dR);
                else
                    dist = static_cast<float>(ORBmatcher::DescriptorDistance(dL, dR));
        ...
        if( (useSP && bestDist < thSPDist) || (!useSP && bestDist < thOrbDist) )
        {
